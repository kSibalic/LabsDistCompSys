...
void handleClient(int clientSocket) {
  ...
  if (procedure == PROCEDURE_SET) {
    ...
    // Read Key and Value
    recv(clientSocket, key, KEY_SIZE, 0);
    recv(clientSocket, value, VALUE_SIZE, 0);

    std::cout << "SET " << key << " = " << value << std::endl;
    kvStore[key] = value;

    // Send Ack
    uint8_t response[4 + KEY_SIZE + VALUE_SIZE] = {0}; // 4 + 64 + 64
    ...
    char valueResponse[VALUE_SIZE] = {0};
    strncpy(valueResponse, resultValue.c_str(), VALUE_SIZE - 1);

    // Client expects only VALUE_SIZE bytes back for GET
    send(clientSocket, valueResponse, VALUE_SIZE, 0);
  } else {
